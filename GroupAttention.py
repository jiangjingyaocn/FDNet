...
from Models.Transformer import token_TransformerEncoder
Norm = nn.LayerNorm
...
class GroupAttention(nn.Module):
    def __init__(self, in_dim):
        ...
        self._init_weights()
        self.group_trans = token_TransformerEncoder(embed_dim=384, depth=4, num_heads=8)
    ...
    def forward(self, feas, ds=False, scale=0.5):
        bs, _, _ = feas.shape
        group_feas = rearrange(feas, "b s dim -> (b s) dim").unsqueeze(0)
        # [1, bhw, c ]
        # print("Q",query_feas.shape)
        # pooled_feas = self.msp_block(feas)
        # print("pooled_feas after msp_block",pooled_feas.shape)
        #
        # pooled_feas = rearrange(pooled_feas, "b c n -> (b n) c").unsqueeze(0)
        # print("pooled_feas after rearrange",pooled_feas.shape)
        #
        ...
        attn_out = self.group_trans(group_feas)
        attn_out = rearrange(attn_out.squeeze(), "(b s) dim -> b s dim", b=bs)
        feas = feas + attn_out
        # if self.ffn is not None:
        #     feas = feas + self.drop_path(self.ffn(feas))
        # feas = rearrange(feas.squeeze(), "(b h w) c -> b c h w", b=bs)
        # 就是这些注意力模块了
        return feas
