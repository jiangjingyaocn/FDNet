...
from .Transformer import token_inference, token_TransformerEncoder, token_Transformer, group_token_TransformerEncoder, \
    saliency_token_inference, contour_token_inference, background_token_inference
...
class token_trans(nn.Module):
    ...
    def forward(self, fea, saliency_tokens, contour_tokens, background_tokens):
        ...
        saliency_fea,s_attn = self.saliency_token_pre(fea)
        # saliency_fea [B, H*W, 384]
        contour_fea,c_attn = self.contour_token_pre(fea)
        # contour_fea [B, H*W, 384]
        background_fea,b_attn = self.background_token_pre(fea)
        ...
        return saliency_fea, contour_fea, background_fea, fea, saliency_tokens, contour_tokens, background_tokens,s_attn,c_attn,b_attn
        # return saliency_fea, contour_fea, background_fea, fea, group_saliency_tokens, group_contour_tokens, background_tokens


class decoder_module(nn.Module):
    def __init__(self, dim=384, token_dim=64, img_size=256, ratio=8, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), fuse=True):
...
class Decoder(nn.Module):
    def __init__(self, embed_dim=384, token_dim=64, depth=2, img_size=256):
        ...
        self.decoder3_b = decoder_module(dim=embed_dim, token_dim=token_dim, img_size=img_size, ratio=1, kernel_size=(7, 7), stride=(4, 4), padding=(2, 2), fuse=False)
        # token based multi-task predictions

        self.token_trans_1_16 = token_Transformer(embed_dim=384, depth=4, num_heads=6, mlp_ratio=3.)
        self.token_trans_1_8 = token_trans(in_dim=token_dim, embed_dim=embed_dim, depth=depth, num_heads=1)
        self.token_trans_1_4 = token_trans(in_dim=token_dim, embed_dim=embed_dim, depth=depth, num_heads=1)
        ...
        # predict background maps
        self.pre_1_16_b = nn.Linear(token_dim, 1)
        self.pre_1_8_b = nn.Linear(token_dim, 1)
        self.pre_1_4_b = nn.Linear(token_dim, 1)
        self.pre_1_1_b = nn.Linear(token_dim, 1)
    ...
    def forward(self,rgb_fea_1_16, rgb_fea_1_8, rgb_fea_1_4):
        ...
        # 加了一个group_token
        saliency_fea_1_16, fea_1_16, saliency_tokens, contour_fea_1_16, contour_tokens , background_fea_1_16 , background_tokens= self.token_trans_1_16(rgb_fea_1_16)
        saliency_fea_1_16 = self.mlp(self.norm(saliency_fea_1_16))
        contour_fea_1_16 = self.mlp_c(self.norm_c(contour_fea_1_16))
        background_fea_1_16 = self.mlp_b(self.norm_c(background_fea_1_16))
        ...
        background_1_16 = self.pre_1_16_b(background_fea_1_16)
        background_1_16 = background_1_16.transpose(1, 2).reshape(B, 1,self.img_size // 16, self.img_size // 16)

        # 1/16 -> 1/8
        # reverse T2T and fuse low-level feature
        fea_1_8 = self.decoder1(fea_1_16[:, 1:-2, :], rgb_fea_1_8)

        # token prediction
        saliency_fea_1_8, contour_fea_1_8, background_fea_1_8, token_fea_1_8, saliency_tokens, contour_tokens , background_tokens,_,_,_ = self.token_trans_1_8(fea_1_8, saliency_tokens, contour_tokens,background_tokens)
        ...
        background_1_8 = self.pre_1_8_b(background_fea_1_8)
        background_1_8 = background_1_8.transpose(1, 2).reshape(B, 1, self.img_size // 8, self.img_size // 8)


        # 1/8 -> 1/4
        fea_1_4 = self.decoder2(token_fea_1_8[:, 1:-2, :], rgb_fea_1_4)

        # token prediction
        saliency_fea_1_4, contour_fea_1_4, background_fea_1_4, token_fea_1_4, saliency_tokens, contour_tokens, background_tokens,s_attn,c_attn,b_attn= self.token_trans_1_4(fea_1_4, saliency_tokens, contour_tokens,background_tokens)
        ...
        background_1_4 = self.pre_1_4_b(background_fea_1_4)
        background_1_4 = background_1_4.transpose(1, 2).reshape(B, 1, self.img_size // 4, self.img_size // 4)

        # 1/4 -> 1
        saliency_fea_1_1 = self.decoder3(saliency_fea_1_4)
        contour_fea_1_1 = self.decoder3_c(contour_fea_1_4)
        background_fea_1_1 = self.decoder3_b(background_fea_1_4)
        ...
        background_1_1 = self.pre_1_1_b(background_fea_1_1)
        background_1_1 = background_1_1.transpose(1, 2).reshape(B, 1, self.img_size // 1, self.img_size // 1)

        # return [mask_1_16, mask_1_8, mask_1_4, mask_1_1], [contour_1_16, contour_1_8, contour_1_4, contour_1_1], [background_1_16, background_1_8, background_1_4, background_1_1],[saliency_fea_1_1,contour_fea_1_1,background_fea_1_1]
        return [mask_1_16, mask_1_8, mask_1_4, mask_1_1], [contour_1_16, contour_1_8, contour_1_4, contour_1_1], [background_1_16, background_1_8, background_1_4, background_1_1],[s_attn,c_attn,b_attn]
